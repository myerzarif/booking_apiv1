...
def send_email_celery(*args, **kwargs):
    if celery_up():
        ...
        return
    logger.warning(
        "Celery workers are not active, trying to send email blocking!")
    send_email(*args, **kwargs)
...
def send_sms_async(*args, **kwargs):
    ...
    if settings.ENVIRONMENT_APP == 'DEVELOPE':
        logger.info('bypass send sms in develop env')
...
def send_sms_celery(*args, **kwargs):
    if celery_up():
        ...
        return
    logger.warning(
        "Celery workers are not active, trying to send sms blocking!")
    send_sms(*args, **kwargs)
