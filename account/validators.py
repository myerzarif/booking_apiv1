...
def password_validator(value):
    ...
    password_pattern = re.compile(
        r'^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9@#$%^&+=]{8,140}$')
    if not password_pattern.match(value):
        raise exceptions.ValidationError(
            "Password length must be at least 8 characters and consists one upercase, one lowercase and one digit!")
    # password_validation.validate_password(value)
    try:
        password_validation.validate_password(value)
    except exceptions.ValidationError as error:
        raise exceptions.ValidationError(
            "The new password is not met password policies!")
...
def address_validator(value):
    ...
    address_pattern = re.compile(
        r'^[a-zA-Z0-9@!\#\-\_\\ \$\%\^\*\&\(\)]{10,1000}$')
    if not address_pattern.match(value):
        raise exceptions.ValidationError(
            "Address length must be at least 10 characters and consists of english alphanumerics!")


def validate_email(loggedUser, email):
    if loggedUser != email:
        raise exceptions.ValidationError(
            "You can not change password of other users!")
...
class NumberValidator(object):
    ...
    def validate(self, password, user=None):
        if not len(re.findall('\d', password)) >= self.min_digits:
            raise exceptions.ValidationError(
                "The password must contain at least {0} digit(s), 0-9.".format(
                    self.min_digits),
